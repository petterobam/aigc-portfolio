...
async function main() {
  console.log('🚀 开始分析番茄小说发布流程（多步骤 V2）');
  ...
  try {
    ...
    try {
      ...
      if (publishButtons.length === 0) {
        ...
        if (allButtons.length > 0) {
          ...
          try {
            ...
            // 保存完整结构
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const structurePath = path.join(outputDir, `publish-page-multi-step-v2-${timestamp}.json`);
      ...
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const structurePath = path.join(outputDir, `publish-page-multi-step-v2-partial-${timestamp}.json`);
