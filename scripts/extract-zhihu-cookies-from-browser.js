...
/**
 * extract-zhihu-cookies-from-browser.js
 *
 ...
 *
 * 使用方法：
 *   node scripts/extract-zhihu-cookies-from-browser.js [--port 9222] [--url http://localhost:9222]
 *
 * 输出：
 *   cookies/zhihu-live-session-<timestamp>.json   ← 知乎 cookie（含 httpOnly）
 *   cookies/zhihu-latest.json                    ← 软链接/最新副本，供脚本直接引用
 *
 * 其他脚本加载方式：
 *   const { loadLatestZhihuCookies } = require('./scripts/extract-zhihu-cookies-from-browser');
...
const COOKIES_DIR       = path.join(WORKSPACE_DIR, '知乎自动运营/🛠️自动化系统/auth');
const LATEST_FILE        = path.join(COOKIES_DIR, 'zhihu-cookies-latest.json');
  ...
  try {
    ...
    for (let i = 0; i < contexts.length; i++) {
      ...
      if (zhihuCookies.length > 0) {
        console.log(`   上下文 ${i + 1}: 找到 ${zhihuCookies.length} 个知乎 cookie`);
        allZhihuCookies.push(...zhihuCookies);

        if (zhihuCookies.length > zhihuCookieCount) {
          zhihuCookieCount = zhihuCookies.length;
          bestContext = context;
        }
      }
    }
    ...
    if (allZhihuCookies.length === 0) {
      throw new Error('未找到知乎相关 cookie。请确认 Chrome 中知乎已处于登录状态。');
    }

    console.log(`📚 共提取 ${allZhihuCookies.length} 个知乎 cookie`);

    // 检查登录态
    const sessionValid = SESSION_COOKIE_NAMES.some(name =>
      allZhihuCookies.some(c => c.name === name && c.value)
    );
    ...
    if (!sessionValid) {
      ...
      const sid = allZhihuCookies.find(c => SESSION_COOKIE_NAMES.includes(c.name));
    ...
    const httpOnlyCount = allZhihuCookies.filter(c => c.httpOnly).length;
    console.log(`   httpOnly cookie：${httpOnlyCount} 个（含登录 token）`);
    console.log(`   普通 cookie：${allZhihuCookies.length - httpOnlyCount} 个`);

    // 保存到文件
    ensureDir(COOKIES_DIR);
    const outFile = path.join(COOKIES_DIR, `zhihu-live-session-${timestamp()}.json`);
    fs.writeFileSync(outFile, JSON.stringify(allZhihuCookies, null, 2), 'utf8');
    console.log(`\n💾 已保存：${outFile}`);

    // 同时覆写 latest.json，方便其他脚本直接引用
    fs.writeFileSync(LATEST_FILE, JSON.stringify(allZhihuCookies, null, 2), 'utf8');
    console.log(`🔗 已更新：${LATEST_FILE}`);

    return { cookies: allZhihuCookies, sessionValid, file: outFile };
 ...
 * @param {string} [cookieFile]  指定 cookie 文件，不传则使用 zhihu-latest.json
...
async function loadLatestZhihuCookies(context, cookieFile) {
  ...
  if (!fs.existsSync(file)) {
    ...
    console.warn('   请先运行：node scripts/extract-zhihu-cookies-from-browser.js');
    return 0;
  }
...
async function main() {
  ...
  if (!available) {
    ...
    console.error('    node scripts/extract-zhihu-cookies.js');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('\n  下一步操作：');
    console.log('    node scripts/verify-zhihu-cookies.js     # 验证 Cookie');
    console.log('    node scripts/publish-zhihu.js           # 发布内容');
    console.log('');
...
module.exports = { extractCookiesViaCDP, loadLatestZhihuCookies, checkCookieExpiry };
