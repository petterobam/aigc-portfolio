 ...
 *
 * 从已运行的 Chrome 浏览器提取知乎 Cookie（通过 Playwright CDP）
 *
 * 使用前提：
 ...
 *
 * 输出：
 *   cookies/zhihu-session-<timestamp>.json   ← 知乎 cookie（含 httpOnly）
...
const LATEST_FILE      = path.join(COOKIES_DIR, 'zhihu-latest.json');
...
async function extractZhihuCookiesViaCDP(port = DEFAULT_CDP_PORT) {
  ...
  try {
    ...
    if (!sessionValid) {
      console.warn('⚠️  未检测到有效登录态 cookie（d_c0 / z_c0 等）');
    ...
    const outFile = path.join(COOKIES_DIR, `zhihu-session-${timestamp()}.json`);
    fs.writeFileSync(outFile, JSON.stringify(zhihuCookies, null, 2), 'utf8');
    console.log(`\n💾 已保存：${outFile}`);

    // 同时覆写 zhihu-latest.json，方便其他脚本直接引用
    const latestFile = path.join(COOKIES_DIR, 'zhihu-latest.json');
    fs.writeFileSync(latestFile, JSON.stringify(zhihuCookies, null, 2), 'utf8');
...
/**
 * 将 zhihu-latest.json 中的 cookie 注入到 Playwright BrowserContext。
...
/**
 * 检查 zhihu-latest.json 中的登录态 cookie 是否过期。
 * @returns {{ valid: boolean, expiresAt: Date|null, daysLeft: number }}
 */
function checkZhihuCookieExpiry() {
  const latestFile = path.join(WORKSPACE_DIR, 'cookies', 'zhihu-latest.json');
  if (!fs.existsSync(latestFile)) {
    return { valid: false, expiresAt: null, daysLeft: 0 };
  }
  const cookies = JSON.parse(fs.readFileSync(latestFile, 'utf8'));
...
async function main() {
  ...
  try {
    ...
    console.log(`  快捷引用  ：${path.join(WORKSPACE_DIR, 'cookies', 'zhihu-latest.json')}`);
    ...
    console.log('\n  💡 下一步操作：');
    console.log('    1. 在 Chrome 中访问 https://www.zhihu.com 并登录');
    console.log('    2. 重新运行此脚本：node scripts/extract-zhihu-cookies-playwright.js');
    console.log('    3. 使用 Cookie 访问知乎：加载 zhihu-latest.json');
    console.log('');
...
module.exports = { extractZhihuCookiesViaCDP, loadZhihuCookies, checkZhihuCookieExpiry };
