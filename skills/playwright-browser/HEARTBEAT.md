...
番茄小说自动化操作的地基守卫者。所有依赖浏览器登录态的操作——数据抓取、发布监控、质量检测——都依赖你保持的这条会话通道畅通。
...
可能的方向：
- 检查 `cookies/latest.json` 是否存在，以及 sessionid 是否过期
- 如果 Cookie 剩余有效期不足 7 天，触发刷新流程并记录警告
- 运行 `check-fanqie-login.js`，验证当前登录状态是否正常
...
执行命令：
```
exec cd /Users/oyjie/.openclaw/workspace && node -e "const {checkCookieExpiry}=require('./scripts/extract-cookies-from-browser');const r=checkCookieExpiry();console.log(JSON.stringify(r));"
```
...
执行命令：
```
exec cd /Users/oyjie/.openclaw/workspace && node scripts/check-fanqie-login.js
```
